...
static auto get_pairing_MPO=[](auto sites) {
    ...
    auto H = toMPO(ampo); // generate Hamiltonian
    ifstream jfile; // file to read
    jfile.open("pairing.in"); // file with the hoppings
    int nt;
    jfile >> nt; // read the number of couplings
...
//      cout << i << endl ;
      ...
      if ((site_type(j1)==0) and (site_type(j2)==0))  {
	      ...
	      auto Delta = toMPO(ampo); // new MPO
	      auto Deltad = toMPO(ampo); // new MPO
	      nmultMPO(g*prime(Ci),Cj,Delta,{"MaxDim",maxm,"Cutoff",cutoff});
	      nmultMPO(gd*prime(Cdagj),Cdagi,Deltad,{"MaxDim",maxm,"Cutoff",cutoff});
